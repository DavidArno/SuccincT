...
namespace SuccincTTests.SuccincT.Tuples
{
    ...
    internal class TupleMatchebleTestsT1T2T3
    {
        ...
        private class TestClass : ITupleMatchable<int, string, Colors>
        {
            ...
            public string B;
            public Colors C;
            public Tuple<int, string, Colors> PropertiesToMatch { get { return Tuple.Create(A, B, C); } }
        }
        ...
        {
            var tuple = new TestClass { A = 1, B = "a", C = Colors.Red };
            tuple.Match().To<int>().With(2, "a", Colors.Green).Or(1, "b", Colors.Blue).Do((x, y, z) => x).Result();
        }
