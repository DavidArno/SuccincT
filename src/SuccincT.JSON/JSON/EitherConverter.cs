...
namespace SuccincT.JSON
{
    public class EitherConverter : JsonConverter
    {
                                        ...
                                        JsonSerializer serializer)
        {
            ...
            var rawIsLeft = jsonObject["isLeft"] ?? throw new JsonException("No 'isLeft' found for \"Either\" value.");
            var isLeft = rawIsLeft.ToObject<bool>(serializer);

            var valueJson = jsonObject["value"] ?? throw new JsonException("No 'value' found for \"Either\" value.");
