...
namespace SuccincT.Unions.PatternMatchers
{
        ...
        IActionWhereHandler<TMatcher, T>
    {
        ...
        {
            _recorder((x, y) => true, null, null, action.ToUnitFuncCastAs<T, TResult>());
            return _matcher;
        }
        ...
        TMatcher IActionWithHandler<TMatcher, T>.Do(Action<T> action)
        {
            _recorder((x, y) => y.Any(value => EqualityComparer<T>.Default.Equals(x, value)),
                      null,
                      _ofValues,
                      action.ToUnitFuncCastAs<T, TResult>());
            return _matcher;
        }

        TMatcher IActionWhereHandler<TMatcher, T>.Do(Action<T> action)
        {
            _recorder(null, _whereExpression, null, action.ToUnitFuncCastAs<T, TResult>());
            return _matcher;
        }
    }
}